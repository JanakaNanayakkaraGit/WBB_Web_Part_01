...
<div class="card multisteps-form__panel p-4 border-radius-xl bg-white js-active">
  ...
  <table class="table" id="form_image_list">
  <thead >
  <tr>
  <th>Id</th>
  <th>Path</th>
...
var table = $('#form_image_list').DataTable({

processing: true,
serverSide: true,
ajax: "{{ route('admin.imagedata') }}",
columns: [
    {data: 'id', name: 'id'},
...
$('#form_image_list').DataTable().ajax.reload();
},
...
let btn = document.getElementById("add");
    btn.addEventListener('click', event => {
        $('#loading').modal('show');
          var upload_image = $('#image_form')[0];
          var upload_image_ajax = new FormData(upload_image);

          $.ajax({
            type:'POST',
            url: "{{ route('admin.ajaxaddimage') }}",
             data: upload_image_ajax,
             contentType: false,
             processData: false,
             success: (response) => {
               if (response) {
